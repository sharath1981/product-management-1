...
public class ProductHandler {
  ...
  public Mono<ServerResponse> findAll(ServerRequest serverRequest) {
    return ServerResponse.ok()
        // .contentType(MediaType.TEXT_EVENT_STREAM) //http GET
        // http://localhost:8080/api/v1/products
        .contentType(
            MediaType.APPLICATION_NDJSON) // http --stream GET http://localhost:8080/api/v1/products
        .body(
            productRepository
                .findAll()
  ...
  public Mono<ServerResponse> findById(ServerRequest serverRequest) {
    ...
    return productRepository
        ...
        .doOnError(
            ex -> log.error("Error while fetching product with id {}: {}", id, ex.getMessage()));
  }
  ...
  public Mono<ServerResponse> update(ServerRequest serverRequest) {
    final var id = getId(serverRequest);
    final var productDtoMono =
        serverRequest.bodyToMono(ProductDto.class).flatMap(validatorUtil::validate);
    return productRepository
        .findById(id)
        .switchIfEmpty(Mono.error(new ProductManagementException(ErrorCode.PRODUCT_NOT_FOUND)))
        .doOnError(
            ex -> log.error("Error while updating product with id {}: {}", id, ex.getMessage()))
  ...
  public Mono<ServerResponse> delete(ServerRequest serverRequest) {
    ...
    return productRepository
        ...
        .switchIfEmpty(Mono.error(new ProductManagementException(ErrorCode.PRODUCT_NOT_FOUND)))
        .doOnError(
            ex -> log.error("Error while deleting product with id {}: {}", id, ex.getMessage()))
        .flatMap(productRepository::delete)
        .then(ServerResponse.noContent().build());
  }

  public Mono<ServerResponse> deleteAll(ServerRequest serverRequest) {
    return productRepository.deleteAll().then(ServerResponse.noContent().build());
  }
  ...
  private UUID getId(ServerRequest serverRequest) {
    try {
      ...
      log.error(
          "Error while getting UUID from id {}: {}",
          serverRequest.pathVariable(AppConstant.ID),
          ex.getMessage());
